"""Generate RelaCaTS-v1 teacher data with a fixed 32-response budget.
...
from relacats_v1.core import (
    canonicalize_answer,
    generate_identity_views,
    generate_option_permutation_views,
)
from relacats_v1.data_creation import dataset_adapter as _dataset_adapter
...
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "relacats_v1/outputs/generated_data"
ANSWER_RE = re.compile(
    r"(?:final\s+)?answer\s*(?:is\s*)?[:=]\s*[\(\[]?([A-E1-5])[\)\]]?",
    flags=re.IGNORECASE,
)
...
def extract_option_answer(response: str, number_of_options: int) -> str | None:
    """Extract the last explicit Answer marker without guessing prose letters."""
    ...
    if token.isdigit():
        ...
        return labels[position] if 0 <= position < len(labels) else None
    return token if token in labels else None


def extract_numeric_answer(response: str) -> str | None:
    """Extract one scalar answer from a GSM8K/SVAMP response.

    Prefer an explicit ``Answer:``/``Final Answer:`` marker.  If a model
    omitted the marker, retain the original CaTS fallback of taking the last
    scalar token.  The value is left as text here; ``canonicalize_answer``
    performs strict finite-number normalization before it can vote.
    """

    text = str(response)
    marked = NUMERIC_MARKER_RE.findall(text)
    if marked:
        return marked[-1].replace(",", "").strip()
    tokens = NUMERIC_TOKEN_RE.findall(text)
    if not tokens:
        return None
    return tokens[-1].replace(",", "").replace("$", "").strip()
