"""Build local, question-grouped RelaCaTS-v1 training JSONL files.
...
DEFAULT_OUTPUT = REPO_ROOT / "relacats_v1/outputs/relssc_dataset"
...
def _validate_v1_weights(payload_id: str, sample: dict[str, Any]) -> None:
    ...
    if relation_weight != 1.0:
        raise ValueError(f"{payload_id}: RelaCaTS-v1 requires r_g=1")
    if dependency_weight != 1.0:
        raise ValueError(f"{payload_id}: RelaCaTS-v1 requires d_gi=1")
...
def validate_question_payload(
    ...
    for index, sample in enumerate(samples):
        ...
        if float(sample.get("relation_weight", 1.0)) != 1.0:
            raise ValueError(f"{payload['question_id']}: RelaCaTS-v1 requires r_g=1")
        if float(sample.get("dependency_weight", 1.0)) != 1.0:
            raise ValueError(f"{payload['question_id']}: RelaCaTS-v1 requires d_gi=1")
...
def build_dataset(
    *,
    ...
    stats = {
        "schema_version": "relacats-v1.dataset-stats.1",
...
def main() -> None:
    ...
    manifest = {
        "schema_version": "relacats-v1.dataset-manifest.1",
        ...
        "target": "relational_consistency == RelSSC(canonicalized_answer)",
        "gold_used_in_target": False,
        "datasets": summaries,
    }
    atomic_write_json(output_root / "manifest.json", manifest)
    print(f"Wrote RelaCaTS-v1 dataset manifest: {output_root / 'manifest.json'}")
