"""Two-task CaTS trainer using RelaCaTS-v1's RelSSC target.
...
from relacats_v1.common import confidence_suffix, read_jsonl
...
def _dataset_fraction(
    ...
    ``train_percentage``/``eval_percentage``.  RelaCaTS-v1 configs use a
...
def prepare_mixed_examples(
    ...
    """Mirror CaTS's confidence-bin + high-SSC SFT sampling locally."""

    dataset_root = resolve_path(config["dataset_root"])
    target_field = config.get("target_field", "relational_consistency")
    ...
    for spec, raw_weight in zip(dataset_specs, raw_weights):
        ...
        for record in records:
            ...
            try:
                ...
                continue
            if not math.isfinite(target_float) or not 0.0 <= target_float <= 1.0:
                continue
        ...
        high_relssc = [record for record in valid if record["_target"] > threshold]
        for record in _sample_without_replacement(high_relssc, causal_count, rng):
            mixed.append(
                {
                    "task": "causal_lm",
                    "dataset_name": name,
...
def main() -> None:
    ...
    if config.get("target_field", "relational_consistency") not in {
        "relational_consistency",
        "relssc",
        "weighted_consistency",
    }:
        raise ValueError("Unsupported calibration target field")
    ...
    if rank == 0:
        ...
        print(f"Saved RelaCaTS-v1 LoRA adapter to {output_dir}", flush=True)
    if dist.is_initialized():
        dist.barrier()
