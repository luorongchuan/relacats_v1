...
# RelaCaTS-v1 retraining launcher for larger per-rank micro-batches.
#
...
OUTPUT_BASE="${OUTPUT_BASE:-${ROOT_DIR}/relacats_v1/outputs}"
...
make_runtime_config() {
  local source="$1" target="$2" model="$3" dataset_root="$4" output="$5" batch="$6" accum="$7" gc="$8"
  mkdir -p "$(dirname "${target}")"
  "${PYTHON_BIN}" - "${source}" "${target}" "${model}" "${dataset_root}" "${output}" "${batch}" "${accum}" "${gc}" <<'PY'
import json
import os
import sys

source, target, model, dataset_root, output, batch, accum, gc = sys.argv[1:]
...
run_one() {
  ...
  local source_config="${ROOT_DIR}/relacats_v1/configs/${CONFIG_NAME}"
  ...
  mkdir -p "${log_dir}"
  make_runtime_config "${source_config}" "${runtime_config}" "${MODEL_NAME}" "${dataset_root}" "${output_dir}" "${BATCH}" "${ACCUM}" "${GC}"
  ...
  if [[ "${DRY_RUN}" == "1" ]]; then
    printf 'DRY RUN:'
    printf ' %q' "${PYTHON_BIN}" -m torch.distributed.run --standalone --nproc_per_node=2 relacats_v1/model_training/train_relacats.py "${args[@]}"
    printf '\n'
    return 0
  fi
  check_gpu_free
  CUDA_VISIBLE_DEVICES="${GPU_FIRST},${GPU_SECOND}" \
    "${PYTHON_BIN}" -m torch.distributed.run --standalone --nproc_per_node=2 \
      relacats_v1/model_training/train_relacats.py "${args[@]}" \
      2>&1 | tee "${train_log}"
...
if [[ "${MODE}" == "smoke" ]]; then
  ...
  for tag in ${MODELS}; do
    run_one "${tag}" "${CHECKPOINT_BASE}/${tag}_relacats_v1" "${LOG_BASE}/${tag}" "" "" ""
  done
  echo "ALL FAST RELACATS-v1 TRAINING RUNS COMPLETE"
fi
