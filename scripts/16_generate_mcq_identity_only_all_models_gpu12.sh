...
# Generate the seven missing MCQ 32I CaTS baselines for all three models.
# GSM8K/SVAMP are intentionally excluded because their complete 32I pools
# already live under generated_data_identity_only.
#
...
DATASETS="${DATASETS:-arc_easy commonsense_qa logiqa openbookqa reclor sciq winogrande}"
OUTPUT_BASE="${OUTPUT_BASE:-${ROOT_DIR}/relacats_v1/outputs/generated_data_identity_only}"
LOG_BASE="${LOG_BASE:-${ROOT_DIR}/relacats_v1/outputs/logs/mcq_identity_gpu${GPU_FIRST}${GPU_SECOND}}"
...
exec 9>"${LOG_BASE}/.mcq_identity_all_models.lock"
flock -n 9 || fail "another MCQ identity launcher is using ${LOG_BASE}"
...
run_worker() {
  ...
  setsid env CUDA_VISIBLE_DEVICES="${gpu}" \
    "${PYTHON_BIN}" -m relacats_v1.data_creation.generate_mcq_identity_only_data \
...
log "MCQ 32I identity-only generation starting"
log "GPUs=${GPU_FIRST},${GPU_SECOND}"
log "datasets=${DATASETS}"
log "profile=1 unchanged MCQ view x 32 completions per question"
log "output_base=${OUTPUT_BASE}"
log "GSM8K/SVAMP are excluded and will not be touched"
...
log "All three models complete. Seven MCQ 32I baselines are populated."
