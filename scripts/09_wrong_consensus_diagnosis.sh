...
OUTPUT_DIR="${OUTPUT_DIR:-${ROOT_DIR}/relacats_v1/outputs/diagnosis/${DATASET_NAME:-unset_dataset}}"
...
"${PYTHON_BIN}" -m relacats_v1.diagnosis.wrong_consensus_diagnosis identify \
...
if (( CANDIDATE_COUNT > 0 )); then
  if [[ "${SKIP_GENERATION}" == "1" ]]; then
    ...
    "${PYTHON_BIN}" -m relacats_v1.data_creation.generate_relational_data \
...
"${PYTHON_BIN}" -m relacats_v1.diagnosis.wrong_consensus_diagnosis compare \
