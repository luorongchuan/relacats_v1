...
MODEL="${MODEL:-${ROOT_DIR}/relacats_v1/outputs/merged_model/qwen2_5_7b_instruct_relacats_v1}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/relacats_v1/outputs/eval_outputs/responses}"
LOG_ROOT="${LOG_ROOT:-${ROOT_DIR}/relacats_v1/outputs/logs/eval_generation}"
...
for dataset in "${DATASETS[@]}"; do
  ...
  echo "Generating ${dataset}: ${num_generations} shared test-time responses/question (for baseline and RelaCaTS aggregators); mode=${EVAL_GPU_MODE}, GPUs=${VISIBLE_GPUS}"
  common=(
    --model "${MODEL}"
    --dataset "${dataset}"
  ...
  if [[ "${EVAL_GPU_MODE}" == "tensor_parallel" ]]; then
    ...
    setsid env CUDA_VISIBLE_DEVICES="${VISIBLE_GPUS}" "${PYTHON_BIN}" \
      -m relacats_v1.evaluation.generate_responses "${common[@]}" --shard-index 0 \
    ...
    setsid env CUDA_VISIBLE_DEVICES="${GPU_SECOND}" "${PYTHON_BIN}" \
      -m relacats_v1.evaluation.generate_responses "${common[@]}" --shard-index 1 \
