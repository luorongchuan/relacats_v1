...
CONFIG_FILE="${CONFIG_FILE:-${ROOT_DIR}/relacats_v1/configs/qwen2_5_7b_2xa100.json}"
...
RUN_ROOT="${RUN_ROOT:-${ROOT_DIR}/relacats_v1/outputs}"
RAW_ROOT="${RAW_ROOT:-${RUN_ROOT}/generated_data}"
DATASET_ROOT="${DATASET_ROOT:-${RUN_ROOT}/relssc_dataset}"
CHECKPOINT_ROOT="${CHECKPOINT_ROOT:-${RUN_ROOT}/checkpoints/qwen2_5_7b_instruct_relacats_v1}"
...
run_generate() {
  ...
  PYTHON_BIN="${PYTHON_BIN}" MODEL_NAME="${MODEL_NAME}" OUTPUT_ROOT="${RAW_ROOT}" \
    ...
    bash "${ROOT_DIR}/relacats_v1/scripts/01_generate_relational_data.sh" 2>&1 | tee -a "${PIPELINE_LOG}"
...
run_smoke() {
  ...
  PYTHON_BIN="${PYTHON_BIN}" MODEL_NAME="${MODEL_NAME}" SMOKE_ROOT="${SMOKE_ROOT}" \
    ...
    bash "${ROOT_DIR}/relacats_v1/scripts/03_smoke_train.sh" 2>&1 | tee -a "${PIPELINE_LOG}"
...
run_build() {
  ...
  PYTHON_BIN="${PYTHON_BIN}" INPUT_ROOT="${RAW_ROOT}" OUTPUT_ROOT="${DATASET_ROOT}" \
    ...
    bash "${ROOT_DIR}/relacats_v1/scripts/02_build_relssc_dataset.sh" 2>&1 | tee -a "${PIPELINE_LOG}"
...
run_train() {
  ...
  PYTHON_BIN="${PYTHON_BIN}" CONFIG_FILE="${RUNTIME_CONFIG}" OUTPUT_DIR="${CHECKPOINT_ROOT}" \
    ...
    bash "${ROOT_DIR}/relacats_v1/scripts/04_train_relacats.sh" 2>&1 | tee -a "${PIPELINE_LOG}"
