...
SMOKE_ROOT="${SMOKE_ROOT:-${ROOT_DIR}/relacats_v1/outputs/smoke}"
...
setsid env CUDA_VISIBLE_DEVICES="${GPU_FIRST}" "${PYTHON_BIN}" -m relacats_v1.data_creation.generate_relational_data \
  "${COMMON[@]}" --shard-index 0 >"${LOG_ROOT}/generation_gpu${GPU_FIRST}.log" 2>&1 &
first_pid=$!
GEN_PIDS+=("${first_pid}")
setsid env CUDA_VISIBLE_DEVICES="${GPU_SECOND}" "${PYTHON_BIN}" -m relacats_v1.data_creation.generate_relational_data \
...
"${PYTHON_BIN}" -m relacats_v1.data_creation.build_relssc_dataset \
  --input-root "${RAW_ROOT}" \
...
CONFIG_FILE="${ROOT_DIR}/relacats_v1/configs/qwen2_5_7b_smoke.json"
...
CUDA_VISIBLE_DEVICES="${GPU_FIRST},${GPU_SECOND}" "${PYTHON_BIN}" -m torch.distributed.run \
  ...
  relacats_v1/model_training/train_relacats.py \
...
"${PYTHON_BIN}" -m relacats_v1.evaluation.synthetic_smoke \
