...
RESPONSES_ROOT="${RESPONSES_ROOT:-${ROOT_DIR}/relacats_v1/outputs/eval_outputs/responses}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/relacats_v1/outputs/eval_outputs/confidence}"
LOG_ROOT="${LOG_ROOT:-${ROOT_DIR}/relacats_v1/outputs/logs/eval_confidence}"
...
PY
  ...
  if [[ "${EVAL_GPU_MODE}" == "tensor_parallel" ]]; then
    ...
    setsid env CUDA_VISIBLE_DEVICES="${VISIBLE_GPUS}" "${PYTHON_BIN}" \
      -m relacats_v1.evaluation.calculate_confidence "${common[@]}" \
