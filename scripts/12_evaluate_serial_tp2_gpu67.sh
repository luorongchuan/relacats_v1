...
BASE_OUTPUT_ROOT="${BASE_OUTPUT_ROOT:-${ROOT_DIR}/relacats_v1/outputs/eval_serial_tp2}"
BASE_LOG_ROOT="${BASE_LOG_ROOT:-${ROOT_DIR}/relacats_v1/outputs/logs/eval_serial_tp2}"
...
echo "RelaCaTS-v1 serial evaluation starting"
...
for spec in "${MODEL_SPEC_ARRAY[@]}"; do
  ...
  response_root="${model_root}/responses"
  confidence_root="${model_root}/confidence"
  ...
  env \
    ...
    SEED="${SEED}" \
    bash "${ROOT_DIR}/relacats_v1/scripts/06_generate_eval.sh"
  wait_gpu_idle
  ...
  env \
    ...
    GPU_MEMORY_UTILIZATION="${GPU_MEMORY_UTILIZATION}" \
    SEED="${SEED}" \
    bash "${ROOT_DIR}/relacats_v1/scripts/07_calculate_confidence.sh"
  wait_gpu_idle
  ...
  env \
    ...
    EXPECTED_QUESTIONS="${EXPECTED_QUESTIONS}" \
    bash "${ROOT_DIR}/relacats_v1/scripts/08_evaluate.sh"

  echo "===== COMPLETE ${tag}: ${result_root} ====="
done

echo
echo "ALL SERIAL RELACATS-V1 EVALUATION RUNS COMPLETE"
