"""Chunked, resumable shared-response generation for RelaCaTS-v1 evaluation.

No relational transformation is applied here.  RelaCaTS-v1 uses relational
...
LOGGER = logging.getLogger("relacats_v1.generate_responses")
        ...
        questions.append(
            {
                ...
                "correct_answer": scalar_json(
                    extract_dataset_answer(dataset_name, str(gold_text), handler)
                ),
...
def generate(args: argparse.Namespace) -> Path:
    ...
    metadata = {
        "schema_version": 1,
        "artifact_type": "relacats_v1_responses",
        ...
        for question, prompt_output in zip(question_batch, outputs):
            ...
            for generation_index, candidate in enumerate(candidates):
                records.append(
                    {
                        "schema_version": 1,
                        "artifact_type": "relacats_v1_response",
