...
from relacats_v1.evaluation._artifacts import (
    complete_chunk,
    merge_chunks,
    require_local_model,
    response_sources,
...
LOGGER = logging.getLogger("relacats_v1.calculate_confidence")
...
def extract_answer(dataset_name: str, text: str, handler: Any) -> Any:
    """Use the dataset handler, with the isolated MathQA parser policy."""

    extracted = extract_dataset_answer(dataset_name, text, handler)
...
def calculate(args: argparse.Namespace) -> Path:
    ...
    metadata = {
        "schema_version": 1,
        "artifact_type": "relacats_v1_confidence",
        ...
        "evaluation_implementation": "RelaCaTS-v1",
    ...
    for batch_index, batch in _selected_batches(
        ...
        for record, output in zip(batch, outputs):
            ...
            if gold is None:
                gold = extract_answer(dataset_name, str(record["gold_text"]), handler)
            elif dataset_name in {"arc_challenge", "arc_easy"}:
                gold = _normalise_arc_label(str(gold))
            answer = extract_answer(dataset_name, str(record["response"]), handler)
            result_records.append(
                {
                    **record,
                    "artifact_type": "relacats_v1_confidence_record",
